...
import {
  ...
  Heart, MessageCircle, Coffee, Mic, Loader2,
} from "lucide-react";
      ...
      {segments.length > 0 && (
        <div className="max-w-3xl mx-auto px-6 py-12">
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-xs tracking-[0.3em] uppercase text-muted-foreground mb-1">What to Expect</p>
              <h2 className="font-serif text-2xl text-foreground">Event Flow</h2>
            </div>
            <span className="text-sm text-muted-foreground">
          ...
          <div className="relative">
            ...
            <div className="space-y-1">
              {segments.map((seg: any, idx: number) => {
                ...
                return (
                  <div key={seg.id} className="relative flex items-start gap-4 pl-12 py-4">
                    {/* Timeline dot */}
                    <div className="absolute left-[18px] top-5 w-3 h-3 rounded-full bg-card border-2 border-gold" />
                    ...
                    <div className="flex-1">
                      ...
                      {seg.description && (
                        <p className="text-muted-foreground/60 text-sm mt-1">{seg.description}</p>
                      )}
                    </div>
                  </div>
                );
